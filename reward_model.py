...
class RewardModel:
    def __init__(self, ds, da, device,
        ...
        self.da = da
        self.device = device
        self.de = ensemble_size  # 1
        self.lr = lr  #
        self.ensemble = []
        self.paramlst = []
        self.opt = None
        self.model = None
        self.max_size = max_size  # 10
    ...
    def train_reward(self):
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                # compute loss
                curr_loss = self.CEloss(r_hat, labels)  # LCE损失
                loss += curr_loss
    ...
    def get_cropping_mask(self, r_hat1, w):
        ...
        for i in range(w):
            B, S, _ = r_hat1.shape
            length = np.random.randint(S-15, S-5+1, size=B)  # 裁剪长度
            start_index_1 = np.random.randint(0, S+1-length)  # 起始位置
        ...
        for epoch in range(num_iters):
            ...
            for member in range(self.de):
                ...
                if self.data_aug_ratio:
                    ...
                    r_hat1 = r_hat1.repeat(self.data_aug_ratio,1,1)  # 第一个维度重复, 第二个维度不变
