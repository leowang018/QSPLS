...
from reward_model_QSPLS import RewardModelQSPLS
...
class Workspace(object):
    def __init__(self, cfg):
        if cfg.wandb:
            ...
            wandb.run.name = f"{cfg.env}_{cfg.experiment}_{cfg.max_feedback}_{cfg.seed}"
        ...
        self.reward_model = RewardModelQSPLS(
            ...
            k_means=cfg.k_means,  # parameter k
            kl_lamda=cfg.kl_lamda, # parameter beta
    ...
    def evaluate(self):
        ...
        for episode in range(self.cfg.num_eval_episodes):  # 10
            obs = self.env.reset()
            self.agent.reset()
            done = False
            episode_reward = 0
    ...
    def learn_reward(self, first_flag=0):
        ...
        if first_flag == 1:
            ...
            labeled_queries = self.reward_model.qspls_sampling()
        ...
        if self.labeled_feedback > 0:
            ...
            if 'metaworld' in self.cfg.env:
                ...
                num_iters = int(
                    np.ceil(self.cfg.reward_update * self.labeled_feedback / self.reward_model.train_batch_size))
    ...
    def train_vae(self):
        num_iters = 50
        vae_loss = self.reward_model.train_vae_model(num_iters)

        if self.cfg.wandb:
            wandb.log({'vae_loss': vae_loss}, step=self.step)
    ...
    def run(self):
        ...
        with tqdm(total=self.cfg.num_train_steps) as pbar:
            while self.step < self.cfg.num_train_steps:
                if done:
                    # evaluate agent periodically         10000
                ...
                if self.step == (self.cfg.num_seed_steps + self.cfg.num_unsup_steps):
                    ...
                    interact_count = 0
                elif self.step > self.cfg.num_seed_steps + self.cfg.num_unsup_steps:  # 10000
...
@hydra.main(config_path='config/train_QSPLS.yaml', strict=True)
def main(cfg):
